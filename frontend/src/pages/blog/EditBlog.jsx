...
const EditBlog = () => {
    ...
    const { data: blogData, loading: blogLoading } = useFetch(`${getEnv('VITE_BASE_API_URL')}/blogs/show/${blogId}`, {
        method: 'GET',
        credentials: 'include'
    }, [blogId])
    ...
    useEffect(() => {
        if (blogData?.blog) {
            const { author, category, title, slug, blogContent, featuredImage, image } = blogData.blog

            setFilePreview(featuredImage ?? image ?? '')
            form.reset({
    ...
    const blogMutation = useMutation({
        mutationFn: async (values) => {
            ...
            const response = await fetch(`${getEnv('VITE_BASE_API_URL')}/blogs/update/${blogId}`, {
                method: 'PUT',
                credentials: 'include',
                body: formData,
            });
