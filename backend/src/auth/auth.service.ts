...
export class AuthService {
    ...
    async googleLogin(name: string, email: string, avatar: string, res: Response) {
        try {
            let user = await this.usersService.findByEmail(email);

            if (!user) {
                const hashedPassword = bcryptjs.hashSync(Math.random().toString());
                user = await this.usersService.create({ name, email, password: hashedPassword, avatar });
            }
